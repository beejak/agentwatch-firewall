...
- Tracewall Cursor skills (the **only** project-knowledge skills — update when
  the PEP story changes; do **not** look at `using-superpowers` for product truth):
  - [`.cursor/skills/tracewall-zta/SKILL.md`](.cursor/skills/tracewall-zta/SKILL.md) — PEP identity, profiles, blast radius, repo map, verify cmds
  - [`.cursor/skills/tracewall-paper/SKILL.md`](.cursor/skills/tracewall-paper/SKILL.md) — evidence→paper, safe numbers, fraud table, paper workflow
  - Other `.cursor/skills/*` are generic Superpowers process copies (brainstorming, TDD, …), not Tracewall domain knowledge.
  - Long-form threat model / operator prose still lives in `SECURITY.md`, `docs/*`, `LESSONS_LEARNED.md` — skills encode agent gates + maps, not a full docs dump.
