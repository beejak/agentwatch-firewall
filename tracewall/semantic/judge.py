"""
tracewall/semantic/judge.py — the semantic detection tier.

The content pre-filter (tier-0) and the deterministic policy DSL (tier-1) match
surface form. The semantic tier scores *intent* on the ambiguous cases the trust
gate escalates — the cases regex and static rules cannot decide (paraphrased
injection, capability abuse that only makes sense in call-tree context, etc.).

Two backends behind one interface:
  - LLM             — an LLM classifier (provider-agnostic, OpenAI-compatible).
...
This tier is async and off the deterministic hot path — invoked only when the
trust gate routes ESCALATE/NARROW.
"""
...
class SemanticJudge:
    ...
    def _llm_enabled(self) -> bool:
        # Opt-out via TRACEWALL_SEMANTIC_LLM=0; otherwise enabled when a key is present.
        return bool(os.environ.get("LLM_API_KEY")) and os.environ.get("TRACEWALL_SEMANTIC_LLM", "1") != "0"
