"""
tracewall/taint/ledger.py — trust / taint / identity store.

A local SQLite-backed ledger holding, per agent_id:
  - trust scores (recovering — no permanent DoS)
  - cross-session taint records (time-decaying)
  - agent identity / capability tokens
  - the multi-hop taint graph (edges)

Reads are cache-backed for hot-path latency; writes are async and never block
enforcement. The fixed-point taint solver is NOT reimplemented here — it lives
in tracewall.taint.mtp.propagate() (single source of truth); this module is the
stateful driver that loads edges, computes per-edge time decay, and writes back.

A clock provider (`now`) is injectable so time-decay is deterministic in tests.
"""
...
from tracewall.core.signal import IdentityCtx, Taint
from tracewall.taint.mtp import GraphEdge, propagate

logger = logging.getLogger(__name__)

# Taint propagation constants
_ρ = 0.8    # decay per hop (read contagion)
_α = 0.02   # trust recovery rate
_β = 0.6    # trust degrade rate
_λ = 0.1    # taint decay per hour

# Multi-hop edge decay weights (MTP)
...
class Ledger:
    """
    Local SQLite-backed store for trust, taint, identity, and the taint graph.

    Parameters
    ----------
    db_path: SQLite file path (caller-supplied; use a tmp file in tests).
    now:     Clock provider returning unix seconds. Injectable for deterministic
             time-decay in tests (defaults to time.time).
    """

    def __init__(self, db_path: str, now: Callable[[], float] = time.time) -> None:
    ...
    async def _ensure_db(self) -> None:
        ...
        try:
            ...
            logger.error("ledger: db init failed: %s", e)
            self._db = None
    ...
    async def get_trust(self, agent_id: str) -> float:
        ...
        if self._db:
            try:
                ...
                logger.warning("ledger: get_trust failed: %s", e)
        return 0.5

    async def on_clean_call(self, agent_id: str, tool: str) -> None:
        """Update trust upward on clean execution (recovering — never stuck at 0)."""
        ...
        if self._db:
            try:
                await self._db.execute(
                    ...
                    (agent_id, score, self._now()),
                )
                await self._db.commit()
            except Exception as e:
                logger.error("ledger: set_trust write failed: %s", e)
    ...
    async def get_taint(self, agent_id: str) -> Optional[Taint]:
        ...
        if self._db:
            try:
                ...
                logger.warning("ledger: get_taint failed: %s", e)
        return None

    def _apply_decay(self, t: Taint) -> Taint:
        """Taint decays at λ/hour since last event."""
        hours_elapsed = (self._now() - t.ts) / 3600.0
    ...
    async def set_taint(self, agent_id: str, taint: Taint) -> None:
        ...
        if self._db:
            try:
                ...
                logger.error("ledger: set_taint write failed: %s", e)
    ...
    async def propagate_read_taint(self, reader_id: str, writer_taint: float) -> None:
        ...
        if new_level > current_level:
            ...
            if new_level >= _QUARANTINE_THRESHOLD:
                logger.warning(
                    "ledger: agent %s taint=%.2f >= quarantine threshold", reader_id, new_level
                )
    ...
    async def get_identity(self, agent_id: str) -> Optional[IdentityCtx]:
        ...
        if self._db:
            try:
                ...
                logger.warning("ledger: get_identity failed: %s", e)
        return None
    ...
    async def register_identity(self, ctx: IdentityCtx) -> None:
        ...
        if self._db:
            try:
                ...
                logger.error("ledger: register_identity failed: %s", e)

    # ── Multi-hop Taint Graph (MTP) ──────────────────────────────────────────
    ...
    async def record_edge(self, src: str, src_type: str, dst: str, dst_type: str,
        ...
        from tracewall.core.signal import EdgeType
        await self._ensure_db()
        if not self._db:
            return
        ...
        ts_val = ts.timestamp() if hasattr(ts, "timestamp") else (ts or self._now())
        try:
            await self._db.execute(
            ...
            logger.error("ledger: record_edge failed: %s", e)

    async def propagate_graph(self, max_hops: int = 8) -> dict:
        """
        Multi-hop Taint Propagation — stateful driver around mtp.propagate().

        Loads current taint levels and edges, computes each edge's elapsed hours
        from the injected clock, delegates the fixed-point solve to
        tracewall.taint.mtp.propagate() (the single source of truth), then writes
        increased taint back for changed nodes.
        """
        await self._ensure_db()
        if not self._db:
            return {}
        ...
        # Load all edges, converting absolute ts → hours_elapsed via the clock
        now = self._now()
        ...
        async with self._db.execute(
            ...
            async for row in cur:
                src, dst, weight, ts_val = row
                hours_elapsed = max(0.0, (now - ts_val) / 3600.0)
                graph.append(GraphEdge(src=src, dst=dst, weight=weight,
        ...
        # Write back increased taint for changed nodes
        for node_id, level in result.changed.items():
            if level > 0.0:
