...
identity; $14/14$) passes beyond banking-only probes. Full \texttt{Firewall.check} latency
...
ASR $0$, utility $1$. Cross-domain stress $14/14$. Mean \texttt{check} $\approx 6.4$\,ms.
...
Argument operators include \texttt{regex}, \texttt{not\_in\_domain}, and
\texttt{matches\_secret\_pattern}.
...
The \texttt{rate\_exceeds} operator is unsupported (never silent).
...
Expected-limit bypasses that still reproduce: leading ZWSP in IBAN, wrong tool-name case.
...
memory contagion, host write paths, and identity/caps ($12$ success $+\,2$ expected\_limit;
$14/14$ pass in \path{robustness_stress.json}).
Successes include blocking Slack-shaped \texttt{send\_message}, \texttt{http\_post}, and
\texttt{upload} after secret reads, plus identity denials when required.
Tracked limits: unknown tool names and PascalCase aliases that miss exact tool-name rules.
...
AgentDojo workspace/travel suites; IBAN Unicode normalization; case-insensitive tool aliases;
org allowlists instead of attacker-IBAN probes; live contagion edges on MCP;
adaptive paraphrase corpus; rate limits that actually work.
